...
public class InfrastructureFactory {
    ...
    private var cachedInfrastructures: [InfrastructureName: Infrastructure]

    private var lastUpdate: [InfrastructureName: Date]
...
//                    }
    ...
    public func metadata(forName name: InfrastructureName) -> Infrastructure.Metadata? {
        return cachedMetadata.first(where: { $0.name == name})
    }

    public func infrastructure(forName name: InfrastructureName) -> Infrastructure? {
        return cachedInfrastructures[name]
    }
    
    private func bundledInfrastructure(withName name: InfrastructureName) -> Infrastructure? {
    ...
    public func update(_ name: InfrastructureName, notBeforeInterval minInterval: TimeInterval?, completionHandler: @escaping ((Infrastructure, Date)?, Error?) -> Void) -> Bool {
    ...
    private func cacheURL(forName name: InfrastructureName) -> URL {
        return WebServices.Endpoint.providerNetwork(name).apiURL(relativeTo: cachePath)
    }
    
    private var bundledMetadataURL: URL? {
        return WebServices.Endpoint.providersIndex.bundleURL(in: Bundle.module)
    }

    // MARK: Modification dates

    public func modificationDate(forName name: InfrastructureName) -> Date? {
    ...
    private func cacheModificationDate(forName name: InfrastructureName) -> Date? {
        let url = cacheURL(forName: name)
        return FileManager.default.modificationDate(of: url.path)
    }

    private func bundleModificationDate(forName name: InfrastructureName) -> Date? {
        guard let url = name.bundleURL else {
            return nil
        }
        return FileManager.default.modificationDate(of: url.path)
...
private extension InfrastructureName {
    var bundleURL: URL? {
        return WebServices.Endpoint.providerNetwork(self).bundleURL(in: Bundle.module)
    }
}
