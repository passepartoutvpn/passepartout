...
public extension Infrastructure.Metadata {
    var guidanceString: String? {
        ...
        let key = "\(prefix).\(name)"
        var format = NSLocalizedString(key, tableName: "Core", bundle: Bundle.module, comment: "")

        // i.e. key not found
        if format == key {
            let purpose = name.credentialsPurpose
            let defaultKey = "\(prefix).default.\(purpose)"
            format = NSLocalizedString(defaultKey, tableName: "Core", bundle: Bundle.module, comment: "")
        }
